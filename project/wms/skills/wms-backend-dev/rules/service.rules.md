...
- 允许使用 partial 按业务维度拆分（如入库/出库/调拨），不按技术操作拆分。
- 单个 partial 文件只承载一个业务维度。
...
- partial 文件**一级按业务维度拆分**（如入库/出库/调拨/质检），二级按操作方向拆分。

一级拆分方向（业务维度）：

- 入库、出库、调拨、质检、上架、拣料、发料等业务领域。

二级拆分方向（操作维度）：

- 查询、创建、更新、删除、校验、扫描、打印、导出、回调、同步。
